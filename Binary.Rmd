---
title: "MAP Prior with Binary Endpoint"
...
The ACR50 rates in the four historical trials are fairly consistent. The random effect meta-analysis yields a point estimate of 0.154. The predictive CI for current control is (0.11, 0.21). 
```{r}
...
## Data Analysis with Observed Current Trial Data
...
```{r}
(prob <- pmixdiff(post_t, post_c,  0, lower.tail=FALSE))
```

To verify, we can draw samples from respective posterior distributions and calculate the probability. It matches that calculated using *pmixdiff*. 
```{r}
